...
If you also run diy-bacnet-server in standalone server-point mode, define object identity directly in its CSV (`Instance` per row) and set analog `CovIncrement` there rather than relying on implicit auto-assignment. That keeps object identifiers stable as points evolve.
